...
@Component({
    selector: 'gl-salary-define-level-table',
    templateUrl: './salary-define-level-table.component.html',
    styleUrls: ['./salary-define-level-table.component.scss'],
})
export class SalaryDefineLevelTableComponent implements OnInit, AfterViewInit {
    isFullScreen = false;
    ...
    causeChangeEditAllIfy = {
        ...
        _loadEditFields: async () => {
            ...
            const scheme = await this.commonService
                .getFieldSchemeConent('sygz_zzdj001')
                .toPromise();
