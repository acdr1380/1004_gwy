...
@Component({
    selector: 'app-salary-standing-change-manager',
    templateUrl: './civil-standing-change-manager.component.html',
    styleUrls: ['./civil-standing-change-manager.component.scss'],
})
export class CivilStandingChangeManagerComponent implements OnInit, AfterViewInit, OnDestroy {
    jobStepStateEnum = JobStepStateEnum;
    /**
     * 路由参数
     */
    ...
    operBaseOptionIfy = {
        ...
        evtArchives: () => {
            this.modalService.confirm({
                nzTitle: '系统提示?',
                nzContent: `<b style="color: red;">确定归档该业务吗？</b>`,
                nzOkText: '确定',
                nzOkType: 'danger',
                nzOnOk: () => {
                    const data = {
    ...
    loadJobMain() {
        this.workflowService.getWfMainData(this.service.wfId).subscribe(result => {
            ...
            this.clientService.buildBreadCrumb([
                ...
                { type: 'text', text: `${result.wfName}-主管审批` },
            ]);
        });
    }
