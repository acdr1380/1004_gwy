...
export class WorkChangeComponent implements OnInit {
    ...
    conditionDrw = {
        ...
        calculation: () => {
            ...
            const data = {
                ...
                // handlerIds工资处理器业务
                handlerIds: ['601'],
                keyIds: [this.personInf[`${this.tableHelper.getTableCode('A01')}_ID`]],
            };
                        ...
                        const dataArray = [
                            {
                                ...
                                keyId: this.personInf[`${this.tableHelper.getTableCode('A01')}_ID`],
                                jobId: this.jobStepInfo.jobId,
                                jobStepId: this.jobStepInfo.jobStepId,
    ...
    saveChange(data, status, event) {
        const resultData = {
            ...
            tableId: status.slice(0, 4) === 'GZ14' ? `${this.tableHelper.getTableCode('GZ14')}` : `${this.tableHelper.getTableCode('A01')}`,
            data: {
                [status]:
