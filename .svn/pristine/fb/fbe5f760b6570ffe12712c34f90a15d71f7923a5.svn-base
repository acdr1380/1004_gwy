...
import { SalaryInitializeStandingChangeComponent } from '../salary-initializel-standing-change/salary-initialize-standing-change.component';
import { SalaryInitializeDrawerComponent } from '../salary-initialize-drawer/salary-initialize-drawer.component';
import { SalaryInitializeService } from '../salary-initialize.service';
import { SalaryInitializeIsTeacherTableComponent } from '../salary-initialize-is-teacher-table/salary-initialize-is-teacher-table.component';

@Component({
    // tslint:disable-next-line:component-selector
    selector: 'salary-initialize-person-table',
    templateUrl: './salary-initialize-person-table.component.html',
    styleUrls: ['./salary-initialize-person-table.component.scss'],
})
export class SalaryInitializePersonTableComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    allChildsDatas = null;
    childIds = ['A01', 'GZ01', 'GZ02', 'GZ06', 'GZ07', 'GZ09', 'GZ10', 'GZ42', 'GZDA07', 'GZA01'];
    ...
    personSelectIfy = {
        ...
        salaryExecuteAll: async () => {
            ...
            this.workService.salaryCalculation(par).subscribe(result => {
                _loading.close();
                if (result) {
    ...
    personListIfy = {
        ...
        evtSelectedPerson: data => {
            ...
            } else if (this.psnBaseInfoIfy.list[this.psnBaseInfoIfy.currentTabIndex].IS_Allow) {
                this.psnSalaryInfoIfy.isUpdateAllow = +new Date();
            }
        },
        /**
    ...
    /**
     * GZA01子集的字段对应的GZ21A03字段数据
     */
    openEditDrawer = {
        sign: null,
    };
    @ViewChild('isTeacherTableComponent', { static: false })
    _isTeacherTableComponent: SalaryInitializeIsTeacherTableComponent;

    // 工龄组件
    @ViewChild('StandingChangeComponent', { static: false })
    _standingChangeComponent: SalaryInitializeStandingChangeComponent;
    ...
    psnBaseInfoIfy = {
        ...
        _initFields: () => {
            this.psnBaseInfoIfy.list.forEach(item => {
                item.fields = innerTransferFields[item.TABLE_CODE];
        ...
        evtClick: (event, field, result) => {
            ...
            if (table_code !== 'GZA01' && field.TABLE_COLUMN_DICTIONARY_CODE) {
            ...
            if (field.TABLE_COLUMN_CODE === 'A0134A') {
                ...
                this.codeListIfy.filterItems = ['05', '06', '07', '08', '09', '10'];
            }
            if (table_code === 'GZA01') {
                // 工资标识信息子集
                if (field.GZ21A03_CODE) {
                    // 将T0159和T0142对应的GZ21A03字段数据传递到是否教护抽屉组件中
        ...
        idCardToDateAndGender: result => {
            ...
            if (Number.isNaN(result['A0107A'])) {
                result['A0107A'] = result.A0184 = result.A0107 = null;
                this.message.warning('身份证号输入错误！');
            }
        },
        // 增加
        ...
        evtSaveChildData: async (param = false) => {
            ...
            if (TABLE_CODE !== 'A01' && !this.personListIfy.selectedPsnData) {
                ...
                return;
            } else if (TABLE_CODE === 'A01' && !child.result.A0101) {
                this.message.warning('请先添加人员主集信息。');
                return;
            ...
            } else if (!child.IS_MAIN && child.PSN) {
                // 保存GZA01工资标识信息
                this.workService.saveChangeData(params).subscribe(result => { });
            } else {
                switch (child.TABLE_CODE) {
                    case 'GZ09':
                        // GZ0904字段数据限制
                        ...
                        // 停薪时间校验
                        if (this.psnBaseInfoIfy.compareGZ0902(params)) {
                            this.message.warning('停薪时间不能早于起薪时间。');
                            return;
                        }
                        break;
                    case 'GZ01':
                        // 毕业时间校验
                        if (this.psnBaseInfoIfy.compareGZ0102(child, params)) {
                            this.message.warning('毕业时间不得小于出生时间。');
                            return;
                        }
                        break;
                }
                this.psnBaseInfoIfy.londing = true;
                const result = await this.service.saveMultipleTableData(params);
                if (!result.data) {
                    this.psnBaseInfoIfy.londing = false;
                } else {
                    this.psnBaseInfoIfy.londing = false;
        ...
        // 判断毕业时间是否在出生时间之后
        compareGZ0102: (child, params) => {
            const dataA01 = this.psnBaseInfoIfy.list.find(item => item.TABLE_CODE === 'A01');
            if (dataA01.result.A0107) {
                return params.some(
                    v => v.data.GZ0102 && new Date(v.data.GZ0102) < new Date(dataA01.result.A0107)
                );
            }
        },
        // 判断停薪时间不能早于起薪时间
        compareGZ0902: params => {
            return params.some(
                v =>
                    v.data.GZ0901 &&
                    v.data.GZ0902 &&
                    new Date(v.data.GZ0902) < new Date(v.data.GZ0901)
            );
        },
        /**
         * 构造子集字段参数
        ...
        getChangeFieldInfo: child => {
            ...
            if (this.sysOuterStatus) {
                ...
                if (child.PSN) {
                    ...
                    // 编辑GZA01工资标识信息时，增加参数
                    if (child.TABLE_CODE === 'GZA01') {
                        params.childId =
                            child.result[`${this.wfTableCode.getTableCode('GZA01')}_ID`];
                    }
                } else {
        ...
        // 根据GZ0232值为GZ0226，GZ0202，GZ0201，GZ0204赋值
        setGZ0232Value: item => {
            if (!item.GZ0232) {
                return;
            }
            const lastStr = item.GZ0232.substring(item.GZ0232.length - 2);
            switch (item.GZ0232.substring(0, 2)) {
                ...
                case '02':
                    item['GZ0226'] = '07';
                    item['GZ0204'] = item.GZ0232.substring(item.GZ0232.length - 1);
                    break;
                case '03':
                    item['GZ0226'] = '05';
                    item['GZ0202'] = '1' + lastStr;
                    break;
                case '04':
                    item['GZ0226'] = '06';
                    item['GZ0202'] = '2' + lastStr;
                    break;
                case '05':
                    item['GZ0226'] = '09';
                    item['GZ0202'] = '3' + lastStr;
                    break;
            }
        },
        /**
        ...
        updatePsnChilds: TABLE_CODE => {
            ...
            this.workService
                ...
                .subscribe(result => {
                    this.psnBaseInfoIfy._buildUpdatePsnChilds(result, TABLE_CODE);
                });
        },
        // 构造数据
        _buildUpdatePsnChilds: (result, tableCode) => {
            const table = this.psnBaseInfoIfy.list.find(item => item.TABLE_CODE === tableCode);
            if (table.PSN) {
                const personInfo = result.find(_v => _v.IS_LAST_ROW);
                Object.assign(personInfo, table.result);
                ...
                if (table.TABLE_CODE === 'GZ01' || table.TABLE_CODE === 'GZ02') {
                    ...
                    const gz02 = this.psnBaseInfoIfy.list
                        ...
                        .result.find(d => d.IS_LAST_ROW);
                    childA01.result['GZ0101'] = gz01.GZ0101 || '';
                    childA01.result['GZ0101_CN'] = gz01.GZ0101_CN || '';
                    childA01.result['GZ0232'] = gz02.GZ0232 || '';
                    childA01.result['GZ0232_CN'] = gz02.GZ0232_CN || '';
                }
                this.psnBaseInfoIfy.pagination._initPage();
            }
        },
    ...
    _wfInfoDrawerElement: SalaryInitializeDrawerComponent;
    ...
    async checkSalary() {
        ...
        this.workService.salaryCalculation(params).subscribe(result => {
            _loading.close();
            if (result) {
                this.psnSalaryInfoIfy.updateSalaryChilds();
                // 需要刷新津补贴表格
        ...
        tableLists.forEach(table => {
            ...
            if (!table.IS_MAIN && table.PSN) {
                if (child) {
                    ...
                    table.fields.forEach(v => {
                        this.formatDataField(v, personInfo);
                        if (table.TABLE_CODE === this.wfTableCode.getTableCode('GZDA07')) {
                            this.setGZDA07FieldsValue(v, personInfo);
                        }
                    });
                    table.result = personInfo || {};
        ...
        switch (field.TABLE_COLUMN_CODE) {
            ...
            case 'GZ0232':
                if (this.allChildsDatas[this.wfTableCode.getTableCode('GZ02')]) {
                    ...
                    data['GZ0232'] = gz02 ? gz02.GZ0232 : '';
                    data['GZ0232_CN'] = gz02 ? gz02.GZ0232_CN : '';
                }
                break;
            default:
                break;
    ...
    setGZDA07FieldsValue(field, data) {
        if (data) {
            switch (field.TABLE_COLUMN_CODE) {
                case 'GZDA0727':
                    data['GZDA0727'] = data['GZDA0727'] === '0' ? '' : data['GZDA0727'];
                    break;
                case 'GZDA0726':
                    data['GZDA0726'] = data['GZDA0726'] === '0' ? '' : data['GZDA0726'];
                    break;
                case 'GZDA0721':
                    data['GZDA0721'] = data['GZDA0721'] === '0' ? '' : data['GZDA0721'];
                    break;
            }
        }
    }
     ...
     */
    updateJBTorGZBS() {
        this.psnBaseInfoIfy.updatePsnChilds('GZA01');
    }
}
