...
(function (global, factory) {
    ...
    var jexcel = function (el, options) {
        ...
        obj.dispatch = function (event) {
            ...
            if (event == 'onafterchanges' && obj.options.persistance) {
                var url =
                    obj.options.persistance == true ? obj.options.url : obj.options.persistance;
        ...
        obj.createCell = function (i, j, value) {
            ...
            if (obj.options.columns[i].editor) {
                ...
                if (obj.options.columns[i].type == 'hidden') {
                    ...
                    td.innerHTML = jSuites.calendar.getDateString(
        ...
        obj.setMerge = function (cellName, colspan, rowspan, ignoreHistoryAndEvents) {
            ...
            if (test) {
                ...
                // 2020年6月11日12:01:27 胡文鸿修改：初始化不选中格子
         ...
         * @return void
         */
        obj.updateCell = function (x, y, value, force, notChange) {
            ...
            if (obj.records[y][x].classList.contains('readonly') == true && !force) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (
                        ...
                        // Update data and cell
                        obj.options.data[y][x] = value;
                        obj.records[y][x].innerHTML = jSuites.calendar.getDateString(
                            formatted ? formatted : value,
                            obj.options.columns[x].options.format
                        );
                    } else if (obj.options.columns[x].type == 'color') {
        ...
        obj.orderBy = function (column, order) {
            if (column >= 0) {
                ...
                if (Object.keys(obj.options.mergeCells).length > 0) {
                    if (
                        !confirm(
                            obj.options.text.thisActionWillDestroyAnyExistingMergedCellsAreYouSure
                        )
                    ) {
                        return false;
                    } else {
        ...
        obj.insertRow = function (mixed, rowNumber, insertBefore) {
            ...
            if (obj.options.allowInsertRow == true) {
                ...
                // Onbeforeinsertrow
                if (
                    obj.dispatch('onbeforeinsertrow', el, rowNumber, numOfRows, insertBefore) ===
                    false
                ) {
                ...
                if (obj.options.search == true) {
                    if (obj.results && obj.results.length != obj.rows.length) {
                        if (
                            confirm(obj.options.text.thisActionWillClearYourSearchResultsAreYouSure)
                        ) {
                            obj.resetSearch();
                        } else {
                            return false;
                ...
                for (var row = rowIndex; row < numOfRows + rowIndex; row++) {
                    ...
                    if (currentRows[0]) {
                        ...
                        if (
                            Array.prototype.indexOf.call(obj.tbody.children, obj.rows[rowNumber]) >=
                            0
                        ) {
                            obj.tbody.appendChild(tr);
                        }
            ...
            if (obj.options.allowDeleteRow == true) {
                if (obj.options.allowDeletingAllRows == true || obj.options.data.length > 1) {
                    ...
                    if (parseInt(rowNumber) > -1) {
                        ...
                        for (var row = rowNumber; row < rowNumber + numOfRows; row++) {
                            if (
                                Array.prototype.indexOf.call(obj.tbody.children, obj.rows[row]) >= 0
                            ) {
        ...
        obj.deleteColumn = function (columnNumber, numOfColumns) {
            ...
            if (obj.options.allowDeleteColumn == true) {
                if (obj.headers.length > 1) {
                    ...
                    if (
                        obj.dispatch('onbeforedeletecolumn', el, columnNumber, numOfColumns) ===
                        false
                    ) {
        ...
        // 2020年6月11日12:03:49 胡文鸿修改： 计算锁定高度
                ...
                for (var i = 0; i < obj.options.freezeRows; i++) {
                    ...
                    for (var j = 0; j < obj.colgroup.length; j++) {
                        if (obj.colgroup[j] && obj.records[i][j]) {
                            var shifted =
                                scrollTop +
                                (i > 0 ? obj.records[i - 1][j].style.height : 0) -
                                1 +
                                'px';
        ...
        if (jexcel.current) {
            if (jexcel.current.edition) {
                if (e.which == 27) {
                    ...
                    // Enter
                    if (
                        jexcel.current.options.columns[jexcel.current.edition[2]].type == 'calendar'
                    ) {
    ...
    jexcel.mouseDownControls = function (e) {
        ...
        if (jexcelTable[0]) {
            ...
            if (jexcel.current) {
                ...
                // 2020年6月11日11:15:34 胡文鸿修改：选中其它位置不取消格子的选中
            ...
            if (jexcel.current.resizing) {
                ...
                if (jexcel.current.resizing.column) {
                    ...
                    if (columns.length > 1) {
                        ...
                        for (var i = 0; i < columns.length; i++) {
                            currentWidth.push(
                                parseInt(jexcel.current.colgroup[columns[i]].getAttribute('width'))
                            );
                        }
                        // Previous width
                    ...
                    // Remove Class
                    jexcel.current.rows[jexcel.current.resizing.row].children[0].classList.remove(
                        'resizing'
                    );
                    var newHeight = jexcel.current.rows[jexcel.current.resizing.row].getAttribute(
                        'height'
                    );
                ...
                if (jexcel.current.dragging) {
                    if (jexcel.current.dragging.column) {
                        ...
                        var columnId = e.target.getAttribute('data-x');
                        // Remove move style
                        jexcel.current.headers[jexcel.current.dragging.column].classList.remove(
                            'dragging'
                        );
                ...
                if (jexcel.current.resizing) {
                    if (jexcel.current.resizing.column) {
                        ...
                        if (jexcel.current.resizing.width + width > 0) {
                            ...
                            jexcel.current.colgroup[jexcel.current.resizing.column].setAttribute(
                                'width',
                                tempWidth
                            );
    ...
    jexcel.methods.engineering = (function () {
        ...
        exports.HEX2BIN = function (number, places) {
            ...
            var negative =
                number.length === 10 && number.substring(0, 1).toLowerCase() === 'f' ? true : false;
        ...
        exports.IMREAL = function (inumber) {
            ...
            // Handle special cases
            if (
                ['i', '+i', '1i', '+1i', '-i', '-1i', 'j', '+j', '1j', '+1j', '-j', '-1j'].indexOf(
                    inumber
                ) >= 0
            ) {
                return 0;
    ...
    jexcel.methods.financial = (function () {
        ...
        exports.DOLLARDE = function (dollar, fraction) {
            ...
            // Add decimal part
            result +=
                ((dollar % 1) * Math.pow(10, Math.ceil(Math.log(fraction) / Math.LN10))) / fraction;
    ...
    jexcel.methods.information = (function () {
        ...
        exports.ISERR = function (value) {
            return (
                [error.value, error.ref, error.div0, error.num, error.name, error.nil].indexOf(
                    value
                ) >= 0 ||
    ...
    jexcel.methods.stats = (function () {
        ...
        exports.Z.TEST = function (range, x, sd) {
            ...
            var n = range.length;
            return (
                1 - exports.NORM.S.DIST((exports.AVERAGE(range) - x) / (sd / Math.sqrt(n)), true)
            );
        };
