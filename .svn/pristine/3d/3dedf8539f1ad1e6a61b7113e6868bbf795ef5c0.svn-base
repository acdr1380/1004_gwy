...
@Component({
    selector: 'tibet-annual-assess-list',
    templateUrl: './tibet-annual-assess-list.component.html',
    styleUrls: ['./tibet-annual-assess-list.component.scss']
})
export class TibetAnnualAssessListComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    personListIfy = {
        ...
        _loadPersonList: async (isRef = false) => {
            ...
            if (!!auditResult && auditResult.length > 0) {
                this.personListIfy.listAll = auditResult.map(p => {
                    return {
                        ...p,
                        text: p.A0101,
                        keyId: p[`${this.tableHelper.getTableCode('A01')}_ID`],
                    }
                });
        ...
        list: [
            {
                name: '公务员获得奖励情况登记表',
                tag: 'b1',
                permission: 'annual_assess_reward',
            },
            {
                name: '公务员年度考核备案登记表',
                tag: 'b2',
                permission: 'annual_assess_recorddjb',
            },
            {
                name: '考核不称职人员落实情况表',
                tag: 'b3',
                permission: 'annual_assess_incompetent',
            },
            {
                name: '考核确定为基本和不称职人员名册',
                tag: 'b4',
                permission: 'annual_assess_basis_incompetent',
            },
            {
                name: '考核确定为优秀等次人员名册',
                tag: 'b5',
                permission: 'annual_assess_excellent',
            },
            {
                name: '上年度考核为基本和不称职人员表',
                tag: 'b6',
                permission: 'annual_assess_lastyear_basis_incompetent',
            },
        ],
        evtChange: ({ index }) => {
