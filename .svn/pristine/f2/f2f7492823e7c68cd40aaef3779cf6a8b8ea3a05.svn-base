...
import { JBT } from '../tibet-person-register-list/fields/JBT';
...
import { TibetPersonRegisterService } from '../tibet-person-register.service';

@Component({
    selector: 'gl-tibet-person-register-table',
    templateUrl: './tibet-person-register-table.component.html',
    styleUrls: ['./tibet-person-register-table.component.scss'],
})
export class TibetPersonRegisterTableComponent implements OnInit, AfterViewInit {
    isFullScreen = false;
    ...
    causeChangeEditAllIfy = {
        ...
        _loadEditFields: async () => {
            ...
            const scheme = await this.commonService
                .getFieldSchemeConent('tibet_person_register_Info')
                .toPromise();
        ...
        save: () => {
            ...
            if (this.commonService.formVerify(this.causeChangeEditAllIfy.form)) {
                const params: any = {
                    ...
                    tableId: `${this.tableHelper.getTableCode('AX2')}`,
                ...
                const paramsArr = list.map(row => {
                    ...
                    params.childId = row[`${this.tableHelper.getTableCode('AX2')}_ID`];
                    return { ...params };
                });
    ...
    causeChangeEditIfy = {
        ...
        _loadEditFields: async () => {
            if (this.causeChangeEditIfy.fields.length > 0) {
                ...
                this.causeChangeEditIfy.fields.forEach(item => {
                    data[item.TABLE_COLUMN_CODE] = this.causeChangeEditIfy.row[
        ...
        save: () => {
            if (this.commonService.formVerify(this.causeChangeEditIfy.form)) {
                ...
                const params = {
                    keyId: row[`${this.tableHelper.getTableCode('A01')}_ID`],
                    childId: row[`${this.tableHelper.getTableCode('AX2')}_ID`],
                    jobId: this.jobStepInfo.jobId,
                    jobStepId: this.jobStepInfo.jobStepId,
                    jobDataId: this.jobStepInfo.jobDataId,
    ...
    personJBTChangeIfy = {
        ...
        row: null,
        _loadJBTChangeList: () => {
            // const { keyId } = this.personJBTChangeIfy.row;
            // const data = {
            //     jobId: this.jobStepInfo.jobId,
            //     jobStepId: this.jobStepInfo.jobStepId,
            //     keyIds: [keyId],
    ...
    constructor(
        ...
        private service: TibetPersonRegisterService
    ) {}
    ...
    private async loadPersonTable() {
        const data = {
            jobId: this.jobStepInfo.jobId,
            jobStepId: this.jobStepInfo.jobStepId,
        };
        const result = await this.workflowService.getWfListData(data).toPromise();
        const param = {
            jobId: this.jobStepInfo.jobId,
            jobStepId: this.jobStepInfo.jobStepId,
            childFields: {},
        };
        param.childFields[`${this.tableHelper.getTableCode('AX2')}`] = [];
        const ax2 = await this.workflowService.getPsnList(this.service.wfId, param).toPromise();
        this.personTableIfy.data = result.map(x => {
            const a = ax2[`${this.tableHelper.getTableCode('AX2')}`].find(
                m =>
                    m.changeState === 0 &&
                    m.IS_LAST_ROW &&
                    m[`${this.tableHelper.getTableCode('AX2')}_A01_ID`] ===
