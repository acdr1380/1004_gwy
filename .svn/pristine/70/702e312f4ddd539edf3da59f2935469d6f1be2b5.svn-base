...
@Component({
    selector: 'gl-tibet-exam-record-start',
    templateUrl: './tibet-exam-record-start.component.html',
    styleUrls: ['./tibet-exam-record-start.component.scss'],
})
export class TibetExamRecordStartComponent implements OnInit, OnDestroy, AfterViewInit {
    /**
     * 路由参数
     */
    ...
    operBaseOptionIfy = {
         ...
         */
        isUsable: false,
        list: <any>[{ name: '填报业务信息' }, { name: '填写公招备案信息' }, { name: '预览业务信息' }],
        current: 0,
        evtStepChange: index => {
    ...
    selectPlan = {
        ...
        visible: false,
        open: () => {
            this.selectPlan.visible = true;
        },
        close: () => {
            this.selectPlan.visible = false;
        }, // 计划表单
        form: new FormGroup({
        ...
        PLAN02Model: null,
        PLAN02onChange: event => {
            if (event) {
                const data = {
                    PLAN02: new Date(event).getFullYear(),
                };
                this.service.getExamPlan(data).subscribe(result => {
                    this.selectPlan.allPlans = result;
                });
            }
        },
    };
    ...
    evtSubmitStart(isEdit = false) {
        ...
        if (!this.workflowService.formVerify(this.selectPlan.form) && !isEdit) {
            return;
        }
        this.selectPlan.close();
        const formData = this.wfBaseParams.form.getRawValue();
        const plan = this.selectPlan.allPlans.find(
            v => v.value === this.selectPlan.form.get('PLAN01').value
        );
