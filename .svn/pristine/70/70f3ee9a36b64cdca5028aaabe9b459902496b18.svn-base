...
@Component({
    selector: 'app-salary-inner-transfer-manager',
    templateUrl: './salary-inner-transfer-manager.component.html',
    styleUrls: ['./salary-inner-transfer-manager.component.scss'],
})
export class SalaryInnerTransferManagerComponent implements OnInit, AfterViewInit, OnDestroy {
    /**
     * 路由参数
     */
        ...
        this.workflowService.getWfMainData(this.service.wfId).subscribe(result => {
            ...
            this.clientService.buildBreadCrumb([
                ...
                },
                {
                    type: 'text',
                    text: `${result.wfName}-${
                        this.URLParams.stepId === 'manager' ? '主管审批' : '人社审批'
                    }`,
                },
            ]);
        });
