...
@Component({
    selector: 'gl-tibet-person-enter-list',
    templateUrl: './tibet-person-enter-list.component.html',
    styleUrls: ['./tibet-person-enter-list.component.scss'],
})
export class TibetPersonEnterListComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    tabBarIfy = {
        ...
        init: () => {
            ...
            this.service.getPersonTableData(params).subscribe(res => {
                this.tabBarIfy.list = res.map(x => ({ ...x, title: `${x.name}( ${x.count} )` }));
                this.tabBarIfy.indexChange(0);
            });
        },
        indexChange: index => {
    ...
    personTableIfy = {
        ...
        loadRows: () => {
            ...
            this.personTableIfy.rows = this.tabBarIfy.current?.tableData.map(x => ({...x, keyId: x[tableId+'_ID']}));
        },
        showFile: item => {
            this._OperFileView.show(item.keyId);
        },
        evtAuditPerson: item => {
            this.auditPersonIfy.title = '人员审批';
            this.auditPersonIfy.isBatch = false;
            this.personTableIfy.current = item;
        ...
        list: [
            {
                name: '公务员信息确认表',
                tag: 'b1',
                permission: 'tibet_person_initialize_001',
                temp: null,
            },
            // {
            //     name: '机关单位工作人员工资变动花名册',
            //     tag: 'b2',
            //     permission: 'wagechange',
            //     temp: null,
            // },
            // {
            //     name: '机关单位工作人员职务变动工资调整审核（统计）表',
            //     tag: 'b3',
            //     permission: 'salaryadjustmentaudit',
            //     temp: null,
            // },
        ],
        evtChange: ({ index }) => {
    ...
    auditPersonIfy = {
        ...
        _initAuditState: () => {
            if (!this.auditPersonIfy.isBatch) {
                const personInfo = this.personTableIfy.current;
        ...
        evtSaveAudit: () => {
            if (this.workflowService.formVerify(this.auditPersonIfy.form)) {
                ...
                if (this.auditPersonIfy.isBatch) {
                    const datas = this.personTableIfy.rows.map(item => {
                        return {
                            ...data,
                            keyValue: item.keyId,
    ...
    showPersonInfoIfy = {
        list: [
            ...
            { label: '下载全部', icon: 'download', tag: 'download-all' },
        ],
        showContent: item => {
            switch (item.tag) {
                case 'file':
                ...
                case 'download-all':
                    const params = {
                        ...
                        FORM_PERMISSION: 'tibet_person_initialize_001',
                        FORM_VERSION: 1,
                    };
                    this.service.batDownloadExcel(params);
                    break;
            }
        },
    };
