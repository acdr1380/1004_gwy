...
@Component({
    selector: 'gl-tibet-person-edit-start',
    templateUrl: './tibet-person-edit-start.component.html',
    styleUrls: ['./tibet-person-edit-start.component.scss']
})
export class TibetPersonEditStartComponent implements OnInit {
    /**
     * 路由参数
     */
        ...
        evtStepChange: index => {
            ...
            switch (index) {
                ...
                case 2:
                    ...
                    this.setAuditUnitIfy.evtWfIsSubmit();
                    break;
            }
        },
    };
    ...
    setAuditUnitIfy = {
        ...
        evtWfIsSubmit: async () => {
            ...
            const result = await this.service.getSubmitState(pd).toPromise();
            // true: 需要直接归档回填，直接调submit方法，submit参数isArchiveAndBackfill设置成true 
            // false: 正常选单位上报
            this.setAuditUnitIfy.wfStatus = result;
        },
        evtWfReferOper: () => {
            if (this.setAuditUnitIfy.wfStatus) {
                this.modalService.confirm({
                    nzTitle: '系统提示?',
                    nzContent: `<b style="color: red;">确定要归档当前业务吗？</b>`,
                    nzOkText: '确定',
                    nzOkType: 'danger',
                    nzOnOk: () => {
                        // 直接归档回填
