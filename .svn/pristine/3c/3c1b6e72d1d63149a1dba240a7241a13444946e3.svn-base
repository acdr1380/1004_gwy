...
import { SalaryCivilOtherReduceService } from '../salary-civil-other-reduce.service';
@Component({
    selector: 'gl-salary-civil-other-reduce-table',
    templateUrl: './salary-civil-other-reduce-table.component.html',
    styleUrls: ['./salary-civil-other-reduce-table.component.scss'],
})
export class SalaryCivilOtherReduceTableComponent implements OnInit {
    isFullScreen = false;
    ...
    personTableIfy = {
        ...
        inputChange: (row, field) => {
            const tableId = field.substr(0, 3);
            const params = {
                keyId: row[`${this.tableHelper.getTableCode('A01')}_ID`],
                // childId: '-1',
                jobId: this.jobStepInfo.jobId,
                ...
                jobDataId: this.jobStepInfo.jobDataId,
                changeType: WfDataChangeTypeEnum.MODIFY,
                tableId: `${this.tableHelper.getTableCode(tableId)}`,
                data: {},
            };
            params.data[field] = row[field];
            if (field === 'A0190' || field === 'A0191') {
                this.service.saveA0190AndA0191(params).subscribe();
            } else {
    ...
    constructor(
        ...
        private service: SalaryCivilOtherReduceService
    ) { }
