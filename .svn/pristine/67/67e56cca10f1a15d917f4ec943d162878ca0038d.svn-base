import { ExcelControlComponent } from './../../../../components/excel-control/excel-control.component';
import {
    Component,
    OnInit,
    AfterViewInit,
...
@Component({
    ...
    selector: 's-level-rise-list',
    templateUrl: './Civil-level-rise-list.component.html',
    styleUrls: ['./Civil-level-rise-list.component.scss'],
})
export class CivilLevelRiseListComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    personListIfy = {
         ...
         * 加载人员列表
         */
        _loadPersonList: async (isRef = false) => {
            ...
            if (!!auditResult && auditResult.length > 0) {
                ...
                // 审批和预览界面只显示已晋升和未晋升人员, 该业务并将其按晋级和晋档，未晋升标签分类
                ...
                if (psnRise.DATA.length === 0 && psnNotRise.DATA.length === 0) {
                    // 不存在已晋升和未晋升人员时
                    this.personListIfy.listAll = [];
                    return;
                }
                this.riesTabIfy.list = psnRise.DATA.map(d => d);
                this.riesTabIfy.list.push(psnNotRise);
    ...
    riesTabIfy = {
        ...
        radioChange: () => {
            ...
            if (item.VALUE === 'IS_NOT_RISE') {
                ...
                item.DATA.forEach(ele => {
                    ele.TABLE_DATA.map(d => {
                        ...
                        this.personListIfy.listAll.push({
                            ...d,
                            text: d.A0101,
                ...
                // 晋级，晋档标签
                this.personListIfy.listAll = item.TABLE_DATA.map(d => {
                    const psn = this.personListIfy.psnAuditData.find(
                        v =>
                            v[`${this.tableHelper.getTableCode('A01')}_ID`] ===
                            d[`${this.tableHelper.getTableCode('A01')}_ID`]
            ...
            this.personListIfy.listAll = [...this.personListIfy.listAll];
            // 人员列表分页
    ...
    formListIfy = {
        ...
        list: [
            ...
            {
                name: '公务员(参公)按年度考核结果晋升级别工资报批表',
                tag: 'b2',
                permission: 'salary_jinjinjindangbpb002',
            },
            {
                name: '公务员(参公)按年度考核结果晋升级别工资花名册',
                tag: 'b3',
                permission: 'salary_jinjijindang001',
            },
            {
                name: '公务员(参公)按年度考核结果晋升档次工资花名册',
                tag: 'b4',
                permission: 'salary_rank_jindanghmc001',
            },
            {
                name: '公务员(参公)按年度考核结果晋升档次工资报批表',
                tag: 'b5',
                permission: 'salary_jindangbpb001',
            },
        ],
        evtChange: ({ index }) => {
                    ...
                    .subscribe(result => {
                        if (result.id) {
                            ...
                            const psn = this.personListIfy.psnAuditData.find(
                                p =>
                                    data.keyValue ===
                                    p[`${this.tableHelper.getTableCode('A01')}_ID`]
                            );
