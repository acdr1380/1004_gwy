...
@Component({
    selector: 'gl-tibet-person-quit-form',
    templateUrl: './tibet-person-quit-form.component.html',
    styleUrls: ['./tibet-person-quit-form.component.scss'],
})
export class TibetPersonQuitFormComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    formListIfy = {
        ...
        list: [
            ...
            {
                name: '参照公务员法管理人员退出汇总表',
                tag: 'b2',
                permission: 'person_quit_collect',
            },
            {
                // name: '登记名称',
                // tag: 'b3',
        ...
        evtChange: ({ index }) => {
            const item: any = this.formListIfy.list[index];
    ...
    personInfoChangeIfy = {
        ...
        loadRows: async () => {
            ...
            // 具体校验的子集
            param.childFields[this.tableHelper.getTableCode('A30')] = [];
            param.childFields[this.tableHelper.getTableCode('A31')] = [];
            param.childFields[this.tableHelper.getTableCode('A75')] = [];
            const result = await this.workflowService
                .getPsnList(this.service.wfId, param)
                .toPromise();
            if (result) {
                this.personInfoChangeIfy.rowsA30 = result[this.tableHelper.getTableCode('A30')]
                    .filter(x => x.changeState >= 0)
                    .map(x => ({
                        ...x,
                        changeState: x.changeState === 0 ? '新增' : '修改',
                    }));
                this.personInfoChangeIfy.rowsA75 = result[this.tableHelper.getTableCode('A75')]
                    .filter(x => x.changeState >= 0)
                    .map(x => ({
                        ...x,
                        changeState: x.changeState === 0 ? '新增' : '修改',
