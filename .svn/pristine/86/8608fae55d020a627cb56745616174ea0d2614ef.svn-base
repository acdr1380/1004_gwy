...
@Component({
    selector: 'gl-exam-record-list',
    templateUrl: './exam-record-list.component.html',
    styleUrls: ['./exam-record-list.component.scss'],
})
export class ExamRecordListComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    selectPlan = {
        ...
        getPlans: () => {
            const data = {
                ...
                jobStepId: this.jobStepInfo.jobStepId,
                childFields: {
                    [`${this.tableHelper.getTableCode('BP01')}`]: [],
                },
            };
            this.workflowService.getPsnList(this.service.wfId, data).subscribe(result => {
        ...
        list: [
            {
                name: '公务员录用审批表',
                tag: 'b1',
                permission: 'tibet_exam_record_employ',
                temp: null,
            },
            {
                name: '公招计划业务需求计划表',
    ...
    personSalaryInfoIfy = {
        ...
        open: () => {
            ...
            param[`${this.tableHelper.getTableCode('A01')}_ID`] = param.keyId;
