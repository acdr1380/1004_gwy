...
import { SalaryCivilStandingChangeService } from '../salary-civil-standing-change.service';
...
@Component({
    selector: 'app-salary-standing-change-start',
    templateUrl: './civil-standing-change-start.component.html',
    styleUrls: ['./civil-standing-change-start.component.scss'],
})
export class CivilStandingChangeStartComponent implements OnInit, AfterViewInit, OnDestroy {
    isFullScreen = false;
    /**
     * 路由参数
     */
    ...
    outStep = {
        ...
        current: 0,
        evtStepChange: index => {
            if (!this.outStep.isUsable) {
                return;
            }
            this.outStep.current = index;
            switch (index) {
                case 1:
                    this.outStep.list[1].template = this._editContent;
                    break;
                case 2:
                    this.outStep.list[2].template = this._preViewTemp;
                    break;
            }
        },
    };
    ...
    tailAfterOper = {
        ...
        loadTailAfterList: () => {
            ...
            this.workflowService
                .selectListByWfTracking(this.URLParams.jobId)
    ...
    constructor(
        ...
        private service: SalaryCivilStandingChangeService,
    ...
    ngAfterViewInit() {
        this.outStep.list = [
            {
                name: '填报业务信息',
                template: this._startContent,
                form: this.wfBaseParams.form,
            },
            {
                name: '填报人员拟确认岗位信息',
            },
    ...
    evtSubmitStart(isEdit = false) {
        ...
        if (isEdit) {
            ...
            this.workflowService.updateStepStandardInfo(params).subscribe(() => {
                this.outStep.isUsable = true;
                this.wfBaseParams.loading = false;
                this.outStep.evtStepChange(1);
            });
            return;
        }
