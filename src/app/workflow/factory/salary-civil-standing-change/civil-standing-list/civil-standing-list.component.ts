...
@Component({
    ...
    selector: 'civil-standing-list',
    templateUrl: './civil-standing-list.component.html',
    styleUrls: ['./civil-standing-list.component.scss'],
})
export class CivilStandingListComponent implements OnInit, AfterViewInit {
    constructor(
            ...
            this.workflowService.getWfListData(data).subscribe(result => {
                if (result.length > 0) {
                    ...
                    this.personSalaryChangeIfy.rowList.forEach(item => {
                        ...
                        item[`New${item.TABLE_COLUMN_CODE}`] =
                            first[`New${item.TABLE_COLUMN_CODE}`];
