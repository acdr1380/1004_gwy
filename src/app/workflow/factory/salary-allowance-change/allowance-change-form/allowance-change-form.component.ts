...
import { SalaryAllowanceChangeService } from '../salary-allowance-change.service';
import { JBT as JBT_SHOW } from '../allowance-change-table/career-allowance-field/JBT_show';

@Component({
    selector: 'salary-allowance-change-form',
    templateUrl: './allowance-change-form.component.html',
    styleUrls: ['./allowance-change-form.component.scss'],
})
export class AllowanceChangeFormComponent implements OnInit, AfterViewInit {
    constructor(
        private cdr: ChangeDetectorRef,
        private workflowService: WorkflowService,
        private commonService: CommonService,
        private tableHelper: WfTableHelper,
        private service: SalaryAllowanceChangeService
    ) {}
        ...
        showContent: item => {
            switch (item.tag) {
                ...
                case 'jbt-sync':
                    ...
                    this.salaryGZDA07JBTChangeIfy.open();
                    break;
                case 'down':
        ...
        _loadSalaryChangeList: () => {
            ...
            this.service.getSpecailWfData(data).subscribe(result => {
                ...
                this.personSalaryChangeIfy.rowList.forEach(item => {
                    ...
                    item[`New${item.TABLE_COLUMN_CODE}_DIFF`] =
                        first[`New${item.TABLE_COLUMN_CODE}_DIFF`];
                });
     ...
     * 津补贴变动情况
     */
    salaryGZDA07JBTChangeIfy = {
        visible: false,
        title: '津补贴变动情况',
        width: 480,
        close: () => (this.salaryGZDA07JBTChangeIfy.visible = false),
        open: () => {
            this.salaryGZDA07JBTChangeIfy._loadJBTChangeList();
            this.salaryGZDA07JBTChangeIfy.visible = true;
        },
        _loadJBTChangeList: () => {
            const { keyId } = this.personListIfy.selectedPerson;
            const data = {
            ...
            this.service.getSpecailWfData(data).subscribe(result => {
                const first = result;
                this.salaryGZDA07JBTChangeIfy.rowList.forEach(item => {
                    item[item.TABLE_COLUMN_CODE] = first[item.TABLE_COLUMN_CODE];
                    item[`New${item.TABLE_COLUMN_CODE}`] = first[`New${item.TABLE_COLUMN_CODE}`];
                    item[`New${item.TABLE_COLUMN_CODE}_DIFF`] =
                        first[`New${item.TABLE_COLUMN_CODE}_DIFF`];
