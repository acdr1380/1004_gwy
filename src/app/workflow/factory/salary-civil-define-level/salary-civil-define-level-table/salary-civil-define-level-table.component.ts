...
@Component({
    selector: 'gl-salary-civil-define-level-table',
    templateUrl: './salary-civil-define-level-table.component.html',
    styleUrls: ['./salary-civil-define-level-table.component.scss'],
})
export class SalaryCivilDefineLevelTableComponent implements OnInit {
    isFullScreen = false;
    ...
    personSelectIfy = {
        ...
        evtChange: () => {
            this.personTableIfy.selectedRowData = null;
            this.loadPersonTable();
        },
        disabledSelectedPsn: () => {
    ...
    causeChangeEditAllIfy = {
        ...
        _loadEditFields: async () => {
            ...
            const scheme = await this.commonService
                .getFieldSchemeConent('zzdjgzyw_gwy01')
                .toPromise();
    ...
    personTableIfy = {
        find: {
            ...
            change: (value: string) => {
                ...
                // 定位选中
                this.personTableIfy.selectedRowData = this.personTableIfy.data[location % pageSize];
            },
            search: (searchKey: string) => {
                if (searchKey) {
            ...
            evtOpenChange: status => {
                if (status) {
                    this.personTableIfy.find.searchKey = null;
                }
            },
        },
        ...
        loadSalaryInfo: row => {
            ...
            const url = `irregularity/oper-salary-info-page;GL=${GL}`;
