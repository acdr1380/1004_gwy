...
@Component({
    selector: 'app-salary-initialize-manager',
    templateUrl: './salary-initialize-manager.component.html',
    styleUrls: ['./salary-initialize-manager.component.scss'],
})
export class SalaryInitializeManagerComponent implements OnInit, AfterViewInit, OnDestroy {
    /**
     * 路由参数
     */
    ...
    operBaseOptionIfy = {
        ...
        evtArchives: () => {
            this.modalService.confirm({
                nzTitle: '系统提示?',
                nzContent: `<b style="color: red;">确定要归档吗？</b>`,
                nzOkText: '确定',
                nzOkType: 'danger',
                nzOnOk: () => {
                    const _loading = this.loading.show();
    ...
    loadJobMain() {
        this.workflowService.getWfMainData(this.service.wfId).subscribe(result => {
            ...
            this.clientService.buildBreadCrumb([
                ...
                {
                    type: 'text',
                    text: `${result.wfName}-${this.URLParams.stepId === 'manager' ? '主管审批' : '人社审批'
                        }`,
                },
            ]);
        });
