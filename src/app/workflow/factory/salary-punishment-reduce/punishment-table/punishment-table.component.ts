...
@Component({
    ...
    selector: 'punishment-table',
    templateUrl: './punishment-table.component.html',
    styleUrls: ['./punishment-table.component.scss'],
})
export class PunishmentTableComponent implements OnInit, AfterViewInit {
    isFullScreen = false;
    ...
    causeChangeEditAllIfy = {
        ...
        _loadEditFields: async () => {
            ...
            const scheme = await this.commonService.getFieldSchemeConent('sygz_cfjj').toPromise();
        ...
        save: () => {
            ...
            if (this.commonService.formVerify(this.causeChangeEditAllIfy.form)) {
                const params: any = {
                    ...
                    tableId: `${this.tableHelper.getTableCode('GZ42')}`,
                    data: this.causeChangeEditAllIfy.form.getRawValue(),
                };
                // GZ4201和GZ4232同步
                // if (params.data.GZ4201) {
                //     params.data.GZ4232 = '01' + params.data.GZ4201;
                // }
                const paramsArr = list.map(row => {
                    params.keyId = row[`${this.tableHelper.getTableCode('A01')}_ID`];
                    params.childId = row.NewGZ42Id;
                    return { ...params };
                });
