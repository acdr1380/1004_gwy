...
@Component({
    selector: 'gl-tibet-person-register-list',
    templateUrl: './tibet-person-register-list.component.html',
    styleUrls: ['./tibet-person-register-list.component.scss'],
})
export class TibetPersonRegisterListComponent implements OnInit, AfterViewInit {
    constructor(
        ...
        list: [
            {
                ...
                temp: null,
                tag: 'b1',
                permission: 'tibet_civil_register_check',
            },
            {
                name: '参照公务员法管理登记表',
                temp: null,
                tag: 'b1',
                permission: 'tibet_person_register_check',
            },
            {
                name: '公务员登记备案表',
                tag: 'b4',
                permission: 'tibet_civil_register_record',
                temp: null,
            },
            {
                name: '参照公务员法管理登记备案表',
                tag: 'b4',
                permission: 'tibet_person_register_record',
                temp: null,
            },
        ],
        evtChange: ({ index }) => {
            ...
            if (!item.temp) {
                ...
                this.showPersonInfoIfy.list.forEach(item => {
                    if (!this.formListIfy.status) {
                        item.status = item.tag === 'down' ? true : false;
                    } else {
                        item.status = true;
                    }
                });
    ...
    personInfoChangeIfy = {
        ...
        open: () => {
            this.personInfoChangeIfy.loadRows();
        ...
        loadRows: async () => {
            const params = {
                ...
                jobStepId: this.jobStepInfo.jobStepId,
                childFields: {},
                keyIds: [this.personListIfy.selectedPerson.keyId],
            };
            // 具体校验的子集
            params.childFields[this.tableHelper.getTableCode('AX2')] = [];
            const result = await this.workflowService
                .getPsnList(this.service.wfId, params)
                .toPromise();
            if (result) {
                this.personInfoChangeIfy.rows = result[this.tableHelper.getTableCode('AX2')]
                    .filter(x => x.changeState >= 0)
                    .map(x => ({
                        ...x,
                        changeState: x.changeState === 0 ? '新增' : '修改',
                    }));
            }
        },
    };
