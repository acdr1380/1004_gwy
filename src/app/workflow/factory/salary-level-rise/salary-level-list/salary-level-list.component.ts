...
@Component({
    selector: 'gl-salary-level-list',
    templateUrl: './salary-level-list.component.html',
    styleUrls: ['./salary-level-list.component.scss'],
})
export class SalaryLevelListComponent implements OnInit, AfterViewInit {
    constructor(
    ...
    riesTabIfy = {
        ...
        radioChange: () => {
            ...
            if (item.VALUE === 'IS_RISE') {
                // 已晋升标签下没有子标签
                this.personListIfy.listAll = item.DATA.map(d => {
    ...
    formListIfy = {
        ...
        list: [
            ...
            {
                name: '正常增加薪级工资花名册',
                permission: 'increase_jinshengxinji001',
                tag: 'b2',
            },
            {
                name: '事业单位工作人员正常增加薪级工资报批（统计）表',
                permission: 'increas_xinjibpb001',
                tag: 'b3',
            },
        ],
        evtChange: ({ index }) => {
