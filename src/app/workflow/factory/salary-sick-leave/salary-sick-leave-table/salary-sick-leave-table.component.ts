...
import { SalarySickLeaveService } from '../salary-sick-leave.service';
@Component({
    selector: 'gl-salary-sick-leave-table',
    templateUrl: './salary-sick-leave-table.component.html',
    styleUrls: ['./salary-sick-leave-table.component.scss'],
})
export class SalarySickLeaveTableComponent implements OnInit, AfterViewInit {
    isFullScreen = false;
    ...
    sickLeave = {
        ...
        cancel: () => {
            if (this.sickLeave.isDisabled) {
                this.message.warning('没有病休假记录，无需取消!');
            }
        },
        isDisabled: false,
        ngModelChange: () => {
    ...
    constructor(
        ...
        private service: SalarySickLeaveService
    ) {}
