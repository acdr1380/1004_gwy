...
@Component({
    selector: 'gl-tibet-level-rise-start',
    templateUrl: './tibet-level-rise-start.component.html',
    styleUrls: ['./tibet-level-rise-start.component.scss'],
})
export class TibetLevelRiseStartComponent implements OnInit, AfterViewInit, OnDestroy {
    /**
     * 路由参数
     */
        ...
        this.activatedRoute.queryParamMap.subscribe(async (params: ParamMap) => {
            ...
            if (params.has('GL')) {
                ...
                this.service.selectByWfId().subscribe(val => {
                    if (val.length > 0) {
                        this.service.selectStepInfo(val[0]).subscribe(val => {
                            this.URLParams = { ...val, isFinished: false };
                            this.canEdit = val.jobStepState === JobStepStateEnum.PROCESSING;
                            this.loadJobMain();
                            this.loadUserContacts();
                            if (this.URLParams?.jobId) {
                                this.loadStepStandardInfo();
