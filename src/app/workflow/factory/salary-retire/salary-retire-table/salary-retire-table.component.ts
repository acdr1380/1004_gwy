...
@Component({
    selector: 'gl-salary-retire-table',
    templateUrl: './salary-retire-table.component.html',
    styleUrls: ['./salary-retire-table.component.scss'],
})
export class SalaryRetireTableComponent implements OnInit, AfterViewInit {
    isFullScreen = false;
    ...
    personSelectIfy = {
        ...
        evtChange: () => {
            this.loadPersonTable();
            this.raiseRetireRatio.getRaiseRetireTable(true);
        },
        psnDataChange: () => {
            // 人员禁选列表
    ...
    personTableIfy = {
         ...
         * 保存表格数据
         */
        saveChangeTable: (data, itemId) => {
            const { jobId, jobStepId, jobDataId } = this.jobStepInfo;
            const param = {
                jobId,
                jobStepId,
                jobDataId,
                keyId: data[`${this.tableHelper.getTableCode('A01')}_ID`],
                childId: data.GZ16Id,
                changeType: 1,
                tableId: `${this.tableHelper.getTableCode('GZ16')}`,
                data: {
                    [itemId]: data[itemId],
                },
            };
            this.workflowService.saveChangeData(param).subscribe();
        },
    };
    ...
    raiseRetireRatio = {
        data: [],
        visible: false,
        selectPerson: { [`${this.tableHelper.getTableCode('A01')}_ID`]: '' },
        open: data => {
            this.raiseRetireRatio.visible = true;
            this.raiseRetireRatio.selectPerson = data;
            this.raiseRetireRatio.getRaiseRetireTable();
        },
        close: () => {
            this.raiseRetireRatio.visible = false;
        },
        addRow: () => {
            this.raiseRetireRatio.data.push({
                GZ1903: '',
                GZ1901: '',
                GZ1902: '',
                GZ1904: '',
            });
            this.raiseRetireRatio.data = [...this.raiseRetireRatio.data];
        },
        loading: false,
        /**
         * 业务中人员所有的数据
         ...
         * 获取表格数据
         */
        getRaiseRetireTable: (isRef = false) => {
            if (this.raiseRetireRatio.personRecords.length > 0 && !isRef) {
                this.raiseRetireRatio.filterPersonRecord();
                return;
            }
            this.raiseRetireRatio.loading = true;
            const { jobId, jobStepId, jobDataId, wfId } = this.jobStepInfo;
            const data = {
                jobId,
                jobStepId,
                jobDataId,
            ...
            data.childFields[`${this.tableHelper.getTableCode('GZ19')}`] = [];
            this.workflowService.getPsnList(wfId, data).subscribe(result => {
                this.raiseRetireRatio.loading = false;
                if (result[`${this.tableHelper.getTableCode('GZ19')}`]) {
                    this.raiseRetireRatio.personRecords = result[
                        `${this.tableHelper.getTableCode('GZ19')}`
                    ].map(v => ({
                        ...v,
                        keyId: v[`${this.tableHelper.getTableCode('GZ19')}_A01_ID`],
                        childId: v[`${this.tableHelper.getTableCode('GZ19')}_ID`],
                    }));
                    this.raiseRetireRatio.filterPersonRecord();
                }
            });
         ...
         */
        filterPersonRecord: () => {
            this.raiseRetireRatio.data = this.raiseRetireRatio.personRecords.filter(
                v =>
                    v.keyId ===
                    this.raiseRetireRatio.selectPerson[`${this.tableHelper.getTableCode('A01')}_ID`]
            );
        },
        /**
         * 保存信息
        ...
        saveChangeTable: () => {
            ...
            const { jobId, jobStepId, jobDataId } = this.jobStepInfo;
            const paramsArr = [];
            this.raiseRetireRatio.data.forEach(v => {
                const param = {
                    jobId,
                    jobStepId,
                    jobDataId,
                    keyId: this.raiseRetireRatio.selectPerson[
                        `${this.tableHelper.getTableCode('A01')}_ID`
                    ],
                    childId: v.childId || -1,
                    changeType: v.childId ? 1 : 0,
                    tableId: `${this.tableHelper.getTableCode('GZ19')}`,
                    data: v,
                };
         ...
         * 删除
         */
        deleteRetireTable: (data, index) => {
            const { jobId, jobStepId, jobDataId } = this.jobStepInfo;
            this.modalService.confirm({
                nzTitle: '系统提示?',
                nzContent: `<b style="color: red;">确定要删除此条数据吗？</b>`,
                nzOkText: '确定',
                nzOkType: 'danger',
                nzOnOk: () => {
                    this.raiseRetireRatio.loading = true;
                    if (!data.childId) {
                        this.raiseRetireRatio.data.splice(index, 1);
                        this.raiseRetireRatio.data = [...this.raiseRetireRatio.data];
                        this.raiseRetireRatio.loading = false;
                        return;
                    }
    ...
    viwePersonExcelIfy = {
        ...
        _setParams: () => {
            ...
            this.viwePersonExcelIfy.params = {
                jobId,
                jobStepId,
                [`${this.tableHelper.getTableCode('A01')}_ID`]: this.viwePersonExcelIfy.row?.[
                    `${this.tableHelper.getTableCode('A01')}_ID`
                ],
            };
        },
