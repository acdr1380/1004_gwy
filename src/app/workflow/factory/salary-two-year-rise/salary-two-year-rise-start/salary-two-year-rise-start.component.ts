...
import { WfParamAttachment } from 'app/workflow/db/WfParamAttachment';
...
@Component({
    selector: 'app-start',
    templateUrl: './salary-two-year-rise-start.component.html',
    styleUrls: ['./salary-two-year-rise-start.component.scss'],
})
export class SalaryTwoYearRiseStartComponent implements OnInit, AfterViewInit, OnDestroy {
    constructor(
    ...
    operBaseInfoIfy = {
        ...
        uploadIfy: {
            fileCustomRequest: item => {
                ...
                this.operBaseInfoIfy.uploadIfy.list.push(file);
    ...
    setAuditUnitIfy = {
        ...
        evtSubmit: () => {
            ...
            this.workflowService.submit(this.service.wfId, data).subscribe(() => {
    ...
    private loadRouterParams() {
        ...
        this.activatedRoute.queryParamMap.subscribe(async (params: ParamMap) => {
            ...
            if (params.has('GL')) {
                ...
                this.isUsable =
                    this.URLParams.jobStepState === JobStepStateEnum.PROCESSING ||
                    this.URLParams.jobStepState === JobStepStateEnum.PENDING;
                // 业务存在
                if (this.URLParams.jobId) {
        ...
        this.workflowService.getStepStandardInfo(this.URLParams).subscribe(result => {
            ...
            if (this.URLParams.jobId) {
                ...
                if (this.jobStepInfo.attachments) {
                    // 设置业务附件
                    this.operBaseInfoIfy.uploadIfy.list = this.jobStepInfo.attachments.map(
                        (item: WfParamAttachment) => {
                            return {
                                operFiles: item,
